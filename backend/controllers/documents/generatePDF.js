...
export const sendDocument = (req, res) => {
 ...
 pdf.create(pdfTemplate(req.body), options).toFile(filepath, (err) => {
  transporter.sendMail({
   ...
   subject: `Document from ${profile.businessName ? profile.businessName : profile.firstName
    }`,
   text: `Document from ${profile.businessName ? profile.businessName : profile.firstName
    }`,
