...
<body>
   ...
   : `<h2>___</h2>`
             ...
             <div class="right">
                 <h2 style="font-size: 38px; font-weight:200px; font-family: Arial, Helvetica, sans-serif;">${Number(balanceDue) <= 0
   ? "Receipt"
   : document?.documentType
     ...
     <section class="row">
       <div class="column">
         ...
         <p style="font-size: 12px">${profile?.address ? profile.address : ""
  }</p>
         <p style="font-size: 12px">${profile?.city ? profile.city : ""}</p>
         <p style="font-size: 12px">${profile?.country ? profile.country : ""
  }</p>
       </div>
       <div class="column">
         <h3 style="font-size: 14px; text-transform: uppercase;"> <strong>Bill To: </strong></h3>
         <p style="font-size: 12px">${document?.customer?.name}</p>
         <p style="font-size: 12px"> <span class="header">No: </span>${document?.customer?.accountNo
  }</p>
         <p style="font-size: 12px"> <span class="header">VAT/TIN No:</span> </>${document?.customer?.vatTinNo
  }</p>
       ...
       <div class="column">
         ...
         <h4 style="font-size: 12px">${totalAmountReceived >= document?.total ? "Paid" : status
  }
     </h4>
     ...
     <section class="summary">
           <table>
             ...
             <tr>
               ...
               <td style="text-align: right; font-size: 10px; font-weight: 700">${document?.currency
  }
                 ${addCurrencyCommas(document?.subTotal.toFixed(2))}
                 </td>
             </tr>
             <tr>
               <td style="font-size: 10px">VAT/Sales Tax (${document?.rates
  }%):</td>
               <td style="text-align: right; font-size: 10px; font-weight: 700">${document?.currency
  }
